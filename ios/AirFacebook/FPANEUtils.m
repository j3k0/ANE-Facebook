...
NSDictionary * FPANE_FREObjectToNSDictionary(FREObject object)
{
    ...
    for (uint32_t i = 0; i < numKeys; i++)
    {
        ...
        switch (type) {
            ...
            {
                NSArray *array = FPANE_FREObjectToNSArrayOfNSString(valueRaw);
                [mutableDictionary setObject:array forKey:key];
                break;
            }
            case PARAM_TYPE_OBJECT:
            {
                NSDictionary *dictionary = FPANE_FREObjectToNSDictionary(valueRaw);
                [mutableDictionary setObject:dictionary forKey:key];
            }
            default:
                continue;
        }
